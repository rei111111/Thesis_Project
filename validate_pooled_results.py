"""Validate a completed pooled run with portable artifact-path comparisons.

Place this file in the project root. It preserves the original scientific source
files and verifies their hashes against the completed training run's receipts.
Its own source hash is recorded separately in the completion report.
"""
...
VALIDATOR_VERSION = "pooled-artifact-paths-v1"


def portable_artifact_path(value: str) -> str:
    """Normalize manifest separators while rejecting non-relative paths."""
    if not isinstance(value, str) or not value or "\x00" in value:
        raise ValueError("An artifact path must be a nonempty relative string.")
...
def verify_artifact_manifest(
    ...
    artifact_path_list = [portable_artifact_path(artifact["path"]) for artifact in artifacts]
    ...
    for paths in training_requirements.values():
        required_paths.update(path.relative_to(root).as_posix() for path in paths)
    ...
    for artifact in artifacts:
        path = root / portable_artifact_path(artifact["path"])
...
def main() -> None:
    ...
    report = {
        ...
        "validation_tool": {
            ...
            "original_validator_source_sha256": file_sha256(
                PROJECT_ROOT / "scripts" / "validate_study_outputs.py"
            ),
            "scientific_code_sha256": scientific_code_manifest(PROJECT_ROOT)["sha256"],
        },
        "source_rows": len(data),
